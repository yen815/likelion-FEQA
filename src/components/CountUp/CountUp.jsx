...
function CountUp({ count: initialCount = 0 }) {
  ...
  const handleCountUp = () => {
    ...
    setCount(nextCount); // f리액트에게 업데이트 요청 ("리액트 다음(re-render) 상태 값은 count + 1이다.", 트리거)

    // setCount(count + 1); // 업데이트 시도
    console.log(count /* currentCount */); // 0 -> 1 = 1 ??? => 여전히 0이다. 바로 업데이트가 안된다.
  };
